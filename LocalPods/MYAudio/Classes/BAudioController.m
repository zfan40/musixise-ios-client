...
- (id)init {
    if ((self = [super init])) {
        ...
        NSCAssert(result == noErr, @"Unable to create an AUGraph object. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        // SAMPLER UNIT
        ...
        NSCAssert(result == noErr,
                  @"Unable to add the Sampler unit to the audio processing graph. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        ...
        NSCAssert(result == noErr,
                  @"Unable to add the Output unit to the audio processing graph. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        ...
        NSCAssert(result == noErr, @"Unable to open the audio processing graph. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        ...
        NSCAssert(result == noErr, @"Unable to obtain a reference to the Sampler unit. Error code: %d '%.4s'",
                  (int)result, (const char *)&result);

        // Load a soundfont into the mixer unit
        [self loadSoundFont:@"AJH_Piano"
                  withPatch:0
        ...
        NSCAssert(result == noErr, @"Unable to obtain a reference to the I/O unit. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);

        result = AUGraphNodeInfo(_processingGraph, mixerNode, 0, &_mixerUnit);
        NSCAssert(result == noErr, @"Unable to obtain a reference to the Mixer unit. Error code: %d '%.4s'",
                  (int)result, (const char *)&result);

        // Set number of busses on mixer
        UInt32 busses = 1;
        result = AudioUnitSetProperty(_mixerUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, 0, &busses,
                                      sizeof(busses));
        NSCAssert(result == noErr, @"Unable to set input busses on mixer. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        ...
        NSCAssert(result == noErr,
                  @"Unable to interconnect the nodes in the audio processing graph. Error code: %d '%.4s'", (int)result,
                  (const char *)&result);
        ...
        NSAssert(result == noErr, @"Unable to initialze AUGraph object. Error code: %d '%.4s'", (int)result,
                 (const char *)&result);

        // Start the graph
        result = AUGraphStart(_processingGraph);
        NSAssert(result == noErr, @"Unable to start audio processing graph. Error code: %d '%.4s'", (int)result,
                 (const char *)&result);
    }
    return self;
}
...
- (void)setInputVolume:(Float32)volume withBus:(AudioUnitElement)bus {
    OSStatus result =
        AudioUnitSetParameter(_mixerUnit, kMultiChannelMixerParam_Volume, kAudioUnitScope_Input, bus, volume, 0);
    NSAssert(result == noErr, @"Unable to set mixer input volume. Error code: %d '%.4s'", (int)result,
             (const char *)&result);
}
...
- (OSStatus)loadFromDLSOrSoundFont:(NSURL *)bankURL
                          withBank:(UInt8)bank
                         withPatch:(int)presetNumber
                       withSampler:(AudioUnit)sampler {
    OSStatus result = noErr;
    ...
    result = AudioUnitSetProperty(sampler, kAUSamplerProperty_LoadPresetFromBank, kAudioUnitScope_Global, 0, &bpdata,
                                  sizeof(bpdata));
...
- (void)setupAudioSession {
    ...
    //    }
    //    else {
    [_audioSession setCategory:AVAudioSessionCategoryPlayback error:&audioSessionError];
...
- (BOOL)isGraphRunning {
    ...
    NSAssert(result == noErr, @"Error trying querying whether graph is running: %d '%.4s'", (int)result,
             (const char *)&result);
