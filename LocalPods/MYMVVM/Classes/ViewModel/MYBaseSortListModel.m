...
- (void)sort {
    ...
    for (MYBaseSortViewModel *model in array) {
        ...
        if (isEmptyString(model.sortedString) || !isAlpha) {
            NSDictionary *dict = [self.sortArray objectAtIndexForALM:0];
            NSMutableArray *dictArray = [dict objectForKey:@"#"];
            [dictArray addObject:model];
            continue;
        }
        int sortCode = asciiCode - 65 + 1;
        NSDictionary *sortDict = [self.sortArray objectAtIndexForALM:sortCode];
...
//        DLog(@"sortArray = %@",sortArray);
    ...
    for (int i = 0; i < self.sortArray.count; i++) {
        NSDictionary *dict = [self.sortArray objectAtIndexForALM:i];
...
- (MYBaseViewModel *)sortViewModelInSection:(NSInteger)section row:(NSInteger)row {
    ...
    NSDictionary *sortDict = [self.sortArray objectAtIndexForALM:section];
    NSArray *array = [[sortDict allValues] objectAtIndexForALM:0];
    return [array objectAtIndexForALM:row];
}
