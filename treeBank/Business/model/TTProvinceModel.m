...
-(void)load{
    @weakify(self)
    [theApiService  postRequest:@"/member/provlist.htm" parameter:@{@"token":[TTRunTime instance].user.token,@"memberid":[TTRunTime instance].user.objId} block:^(id result, BOOL ret, NSError *error) {
        @strongify(self)
        ...
        if( status ){
            ...
            [[NSNotificationCenter defaultCenter]postNotificationName:kTTProvinceDataChangedNotification object:nil];
        }else{
