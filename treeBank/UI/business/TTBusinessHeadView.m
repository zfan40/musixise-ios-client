...
-(void)layoutSubviews{
    ...
    _imageView.frame = CGRectMake(self.width-10-imageSize.width, (self.height-imageSize.height)/2.0, imageSize.width, imageSize.height );
...
-(void)onDealType{
    ...
    if( self.eventDelegate ){
        [self.eventDelegate onEvent:kClickDealResultSort view:nil parameter:@{@"datas":@[@"全部",@"上门收款",@"即时提现"],@"open":@(_dealTypeBtn.buttonSelected)}];
    }
}
...
-(void)onDealPay{
    ...
    if( self.eventDelegate ){
        [self.eventDelegate onEvent:kClickDealResultSort view:nil parameter:@{@"open":@(_dealPayBtn.buttonSelected)}];
    }
...
-(void)onPayState{
    ...
    if( self.eventDelegate ){
        [self.eventDelegate onEvent:kClickDealResultSort view:nil parameter:@{@"datas":@[@"全部",@"待支付",@"支付中",@"支付失败",@"支付成功"],@"open":@(_payStateBtn.buttonSelected)}];
    }
}
...
-(void)onAuditState{
    ...
    if( self.eventDelegate ){
        [self.eventDelegate onEvent:kClickDealResultSort view:nil parameter:@{@"datas":@[@"全部",@"未审核",@"审核中",@"重新签名",@"重新拍照",@"审核拒绝"],@"open":@(_auditStateBtn.buttonSelected)}];
    }
}
