...
- (void)layoutSubviews {
    ...
    CGFloat margin = 10;
    CGSize textSize =
        [_label.text boundingRectWithSize:CGSizeMake(INT16_MAX, self.height)
                                  options:NSStringDrawingUsesFontLeading
                               attributes:@{
                                   NSFontAttributeName: _label.font ? _label.font : [UIFont systemFontOfSize:12]
                               }
                                  context:nil]
            .size;
    self.mainImageView.frame = CGRectMake((size.width - imageSize.width) / 2.0,
                                          (self.height - imageSize.height - margin - textSize.height) / 2.0,
                                          imageSize.width, imageSize.height);
    _label.frame = CGRectMake((self.width - textSize.width) / 2.0, self.mainImageView.bottom + margin, textSize.width,
                              textSize.height);
}
...
- (void)layoutSubviews {
    ...
    _getMoney.frame = CGRectMake(self.width - buttonSize.width - 50, self.height - buttonSize.height - 20,
