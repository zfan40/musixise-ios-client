...
- (void)getDisplayImagesWithCurpage:(NSInteger)page {
    ...
    if (_delegate && [_delegate numberOfPages] > 0) {
        UIView *preView =
            [_delegate bannerScrollView:self viewForPageAtIndexPath:[NSIndexPath indexPathForRow:pre inSection:0]];
        UIView *curView =
            [_delegate bannerScrollView:self viewForPageAtIndexPath:[NSIndexPath indexPathForRow:page inSection:0]];
        ...
        UIView *lastView =
            [_delegate bannerScrollView:self viewForPageAtIndexPath:[NSIndexPath indexPathForRow:last inSection:0]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)aScrollView {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:self.time
                                                  target:self
                                                selector:@selector(onAutoScrollView:)
                                                userInfo:nil
                                                 repeats:YES];
}
...
- (void)onTap:(UITapGestureRecognizer *)tapGesture {
    if (self.delegate && [self.delegate respondsToSelector:@selector(bannerScrollView:didSeclectAtIndexPath:)]) {
        [self.delegate bannerScrollView:self
                  didSeclectAtIndexPath:[NSIndexPath indexPathForRow:_currentPage inSection:0]];
    }
}
