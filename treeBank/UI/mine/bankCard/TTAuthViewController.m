...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ;
        cell = [[TTAuthTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)onClickNext {
    [[TTRouter defaultRouter] route:@"treeBank://interPage/TTAuthPostViewController"
                          withParam:@{
                              @"model": _authModel
                          }];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @weakify(self) if (indexPath.section == 1) {
        if (0 == indexPath.row) {
            void (^bankBlock)(TTBankModel *model) = ^(TTBankModel *model) {
                @strongify(self) self.authModel.accountBankId = model.bankCode;
                self.authModel.accountBankName = model.bankShortname;
                [_tableView reloadData];
            };
            [[TTRouter defaultRouter] route:@"treeBank://interPage/TTBankViewController"
                                  withParam:@{
                                      @"bankBlock": bankBlock
                                  }];
        } else if (1 == indexPath.row) {

            void (^block)(TTProvinceModel *model, TTCityModel *cityModel) =
                ^(TTProvinceModel *model, TTCityModel *cityModel) {
                    self.authModel.provinceCardId = model.provid;
                    self.authModel.cityCardId = cityModel.cityid;
                    self.authModel.areaName =
                        [NSString stringWithFormat:@"%@%@", model.provshortname, cityModel.cityshortname];
                    [_tableView reloadData];

                };

            [[TTRouter defaultRouter] route:@"treeBank://interPage/TTAreaViewController"
                                  withParam:@{
                                      @"provinceBlock": block
                                  }];
        }
