...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (1 == indexPath.section) {
        ...
        if (!cell) {
            cell = [[TTBankCardReviseTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    @weakify(self) if (indexPath.section == 0) {
        if (2 == indexPath.row) {
            void (^bankBlock)(TTBankModel *model) = ^(TTBankModel *model) {
                @strongify(self) self.model.accountBankId = model.bankCode;
            ...
            void (^block)(TTProvinceModel *model, TTCityModel *cityModel) = ^(TTProvinceModel *model,
                                                                              TTCityModel *cityModel) {
