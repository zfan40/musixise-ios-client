...
-(void)onIdCardUpClick{
    [TTPhotoPicker showCameraToAllowsEditing:YES singleSelect:YES block:^(NSArray *image) {
...
-(IBAction)onPost:(id)sender{
    ...
    [imageDics setObject:_personPicImageView.image forKey:@"personfile"];
     ...
     [theApiService postImageRequest:@"/account/certification.htm" parameter:dics imageDics:imageDics block:^(id result, BOOL ret, NSError *error) {
         BOOL status =[[result objectForKey:@"success"]boolValue];
         NSString *message =[result objectForKey:@"message"];
         if(status ){
             [TTTipsHelper showTip:@"实名认证资料提交成功"];
             [self route:@"treeBank://interBack" withParam:@{ @"viewController": @"TTMyMainViewController" }];
         }else{
