...
- (void)viewDidLoad {
    ...
    //    [_headView.leftButton addTarget:self action:@selector(onMessage)
    //    forControlEvents:UIControlEventTouchUpInside];
    //    [_headView.rightButton addTarget:self action:@selector(onLogoOut)
    //    forControlEvents:UIControlEventTouchUpInside];
    //    [self.view addSubview:_headView];
    //    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(onUserChanged)
    //    name:kTTUserChangedNotification  object:nil];
    //    [self onUserChanged];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        cell = [[TTMyMainTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        ...
        [TTActionSheet showActionSheetWithTitle:[NSString stringWithFormat:@"是否拨打电话:400-000-7077"]
            subview:self.view
            cancelButtonTitle:@"取消"
            destructiveButtonTitle:@"立即拨打"
