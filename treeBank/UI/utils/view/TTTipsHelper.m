...
void ontips(TTTipContent *ctx, UIView *view, TTTipFinished block) {
    if (TTTip_Msg == ctx.style) {
        ...
        UIAlertView *alert =
            [UIAlertView alertViewWithTitle:title message:msg cancelButtonTitle:cancelTitle otherButtonTitles:others];
    ...
    if (isEmptyString(text) || isEmptyString(detailText)) {
        ...
        detailText = nil;
        CGSize size = [text boundingRectWithSize:CGSizeMake(INT16_MAX, 30)
                                         options:NSStringDrawingUsesFontLeading
                                      attributes:@{
                                          NSFontAttributeName: HUD.labelFont
                                      }
                                         context:nil]
                          .size;
