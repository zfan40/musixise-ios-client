...
static char firstLetterArray[HANZI_COUNT] =
    ...
    "bdczsxwhamlczcpjmcnhjyjnsykchskqmczqdllkablwjqsfmocdxjrrlyqchjmybyqlrhetfjzfrfksryxfjdwtsxxywsqjysly"
...
+ (NSArray *)getFormattedHanyuPinyinStringArrayWithChar:(unichar)ch
                            withHanyuPinyinOutputFormat:(HanyuPinyinOutputFormat *)outputFormat {
    NSMutableArray *pinyinStrArray =
        [NSMutableArray arrayWithArray:[PinyinHelper getUnformattedHanyuPinyinStringArrayWithChar:ch]];
    if (nil != pinyinStrArray) {
        for (int i = 0; i < (int)[pinyinStrArray count]; i++) {
            [pinyinStrArray
                replaceObjectAtIndex:i
                          withObject:[PinyinFormatter formatHanyuPinyinWithNSString:[pinyinStrArray objectAtIndex:i]
...
+ (NSString *)toHanyuPinyinStringWithNSString:(NSString *)str
    ...
    BOOL bLastPinYin = FALSE;
    for (int i = 0; i < str.length; i++) {
        NSString *mainPinyinStrOfChar = [PinyinHelper getFirstHanyuPinyinStringWithChar:[str characterAtIndex:i]
                                                            withHanyuPinyinOutputFormat:outputFormat];
...
+ (NSString *)getFirstHanyuPinyinStringWithChar:(unichar)ch
    ...
    NSArray *pinyinStrArray =
        [PinyinHelper getFormattedHanyuPinyinStringArrayWithChar:ch withHanyuPinyinOutputFormat:outputFormat];
...
+ (NSString *)getPinYin:(NSString *)str
    ...
    NSMutableString *resultPinyinStrBuf = [[NSMutableString alloc] init];
    BOOL bLastPinYin = FALSE;
    BOOL bLastPinYinSep = FALSE;
    for (int i = 0; i < str.length; i++) {
        unichar c = [str characterAtIndex:i];
        NSString *mainPinyinStrOfChar =
            [PinyinHelper getFirstHanyuPinyinStringWithChar:c withHanyuPinyinOutputFormat:outputFormat];
        if (nil != mainPinyinStrOfChar) {
            if (seperater && bLastPinYin && i)
                [resultPinyinStrBuf appendString:seperater];
            [resultPinyinStrBuf appendString:mainPinyinStrOfChar];
            bLastPinYin = FALSE;
            if (seperater && i != [str length] - 1) {
                [resultPinyinStrBuf appendString:seperater];
            }

            if (shortedPinyin && mainPinyinStrOfChar.length) {
                [shortedPinyin appendFormat:@"%C", [mainPinyinStrOfChar characterAtIndex:0]];
            }
        } else {
            if (isalnum(c)) {
                [resultPinyinStrBuf appendFormat:@"%C", c];
                if (!bLastPinYin || (bLastPinYinSep && bLastPinYin))
                    [shortedPinyin appendFormat:@"%C", c];
                bLastPinYinSep = FALSE;
            } else {
                if (seperater)
                    [resultPinyinStrBuf appendString:seperater];
                bLastPinYinSep = YES;
