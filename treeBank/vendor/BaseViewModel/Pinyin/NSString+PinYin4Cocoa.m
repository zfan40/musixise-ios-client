...
- (NSInteger)indexOfString:(NSString *)s fromIndex:(int)index {
    ...
    NSRange searchRange = NSMakeRange((NSUInteger)index, [self length] - (NSUInteger)index);
...
- (NSString *)stringByReplacingRegexPattern:(NSString *)regex
    ...
    return
        [self stringByReplacingRegexPattern:regex withString:replacement caseInsensitive:ignoreCase treatAsOneLine:NO];
}
...
- (NSString *)stringByReplacingRegexPattern:(NSString *)regex
    ...
    NSRegularExpression *pattern =
        [NSRegularExpression regularExpressionWithPattern:regex options:options error:&error];
    if (error) {
...
- (NSArray *)stringsByExtractingGroupsUsingRegexPattern:(NSString *)regex
    ...
    [pattern enumerateMatchesInString:self
                           ...
                           usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                               // Note, we only want to return the things in parens, so we're skipping index 0
                               // intentionally
