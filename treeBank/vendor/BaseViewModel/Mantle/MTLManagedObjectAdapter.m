...
- (id)modelFromManagedObject:(NSManagedObject *)managedObject
    ...
    BOOL (^setValueForKey)
    (NSString *, id) = ^(NSString *key, id value) {
    ...
    for (NSString *propertyKey in [self.modelClass propertyKeys]) {
        ...
        (NSAttributeDescription *) = ^(NSAttributeDescription *attributeDescription) {
        ...
        (NSRelationshipDescription *) = ^(NSRelationshipDescription *relationshipDescription) {
            ...
            if (nestedClass == nil) {
                [NSException raise:NSInvalidArgumentException
                            format:@"No class specified for decoding relationship at key \"%@\" in managed object %@",
        ...
        (NSPropertyDescription *) = ^(NSPropertyDescription *propertyDescription) {
            if (propertyDescription == nil) {
                if (error != NULL) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(@"No property by name \"%@\" exists on the entity.", @""),
                                         managedObjectKey];
            ...
            if ([propertyClassName isEqual:@"NSAttributeDescription"]) {
                ...
                if (error != NULL) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(@"Property descriptions of class %@ are unsupported.", @""),
                                         propertyClassName];
        ...
        if (error != NULL) {
            NSDictionary *userInfo = @{
                ...
                NSLocalizedFailureReasonErrorKey:
                    [NSString stringWithFormat:NSLocalizedString(@"%1$@ could not be parsed because its entity "
                                                                 @"attribute mapping contains illegal property keys.",
                                                                 nil),
                                               modelClass]
            };
        ...
        if (modelClass == nil) {
            if (error != NULL) {
                NSDictionary *userInfo = @{
                    NSLocalizedDescriptionKey: NSLocalizedString(@"Could not deserialize managed object", @""),
                    NSLocalizedFailureReasonErrorKey:
                        NSLocalizedString(@"No model class could be found to deserialize the object.", @"")
                };
...
- (id)managedObjectFromModel:(MTLModel<MTLManagedObjectSerializing> *)model
        ...
        managedObject = performInContext(context, ^id {
            ...
            if (results == nil) {
                ...
                if (error != NULL) {
                    ...
                    fetchRequestError = [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
    ...
    if (managedObject == nil) {
        managedObject =
            [entityDescriptionClass insertNewObjectForEntityForName:entityName inManagedObjectContext:context];
    } else {
    ...
    if (managedObject == nil) {
        if (error != NULL) {
            NSString *failureReason = [NSString
                stringWithFormat:NSLocalizedString(@"Failed to initialize a managed object from entity named \"%@\".",
                                                   @""),
                                 entityName];
    ...
    [dictionaryValue enumerateKeysAndObjectsUsingBlock:^(NSString *propertyKey, id value, BOOL *stop) {
        ...
        NSManagedObject * (^objectForRelationshipFromModel)(id) = ^id(id model) {
            if (![model isKindOfClass:MTLModel.class] ||
                ...
                tmpError = [NSError errorWithDomain:MTLManagedObjectAdapterErrorDomain
        ...
        (NSRelationshipDescription *) = ^(NSRelationshipDescription *relationshipDescription) {
            ...
            if ([relationshipDescription isToMany]) {
                if (![value conformsToProtocol:@protocol(NSFastEnumeration)]) {
                    NSString *failureReason = [NSString
                        stringWithFormat:NSLocalizedString(
                                             @"Property of class %@ cannot be encoded into a to-many relationship.",
                                             @""),
                                         [value class]];
...
+ (id)managedObjectFromModel:(MTLModel<MTLManagedObjectSerializing> *)model
    ...
    return
        [self managedObjectFromModel:model insertingIntoContext:context processedObjects:processedObjects error:error];
}
...
- (NSValueTransformer *)entityAttributeTransformerForKey:(NSString *)key {
    ...
    if ([self.modelClass respondsToSelector:selector]) {
        NSInvocation *invocation =
            [NSInvocation invocationWithMethodSignature:[self.modelClass methodSignatureForSelector:selector]];
  ...
  forKeysFromManagedObject:(NSManagedObject *)managedObject {
    if ([model respondsToSelector:@selector(mergeValuesForKeysFromManagedObject:)]) {
        ...
        [[model.class managedObjectKeysByPropertyKey]
            enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSString *managedObjectKey, BOOL *stop) {
