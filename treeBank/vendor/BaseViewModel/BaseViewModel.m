...
//    NSLog(@"class:%@ setLoadFinished:%d nextPage:%ld page:%ld isRefresh:%d", NSStringFromClass(self.class), succeed, (long)nextPage, (long)page, self.isRefreshing);
...
-(void)echoForKey:(NSString*)key {
    [self each:^(NSIndexPath *index) {
        NSLog(@"section:%ld row:%ld key:%@ data:%@", (long)index.section, (long)index.row, key, [self data:index key:key]);
    }];
}
