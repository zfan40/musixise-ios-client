...
-(void)remove:(NSIndexPath* const)indexPath count:(NSInteger)count {
    ...
    NSArray* sortedkeys = [[map allKeys] sortedArrayWithOptions:NSSortStable usingComparator:^NSComparisonResult(NSIndexPath* idx1, NSIndexPath* idx2) {
...
-(void)doSortFilter {
    ...
    if (needFilter) {
        BOOL bAppend = FALSE;
        if (!_filter && _lastEnableConverPinYinFilter == _enableConverPinYinFilter && _lastEnableRegExpFileter == _enableRegExpFileter && _lastPatterns &&
            [_patterns hasPrefix:_lastPatterns]) {
...
//                bAppend = TRUE;
        ...
        NSUInteger (*filterFun)(__strong id,SEL,id,id) = (NSUInteger (*)(__strong id, SEL, id, id))(class_getMethodImplementation(self.class, sel));
        
        if (bAppend) {
            for (NSMutableArray* items in _list) {
                NSMutableArray* sectionItems = [NSMutableArray arrayWithCapacity:items.count];
                [items enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(NSIndexPath* index, NSUInteger idx, BOOL *stop) {
                    int keyIndex = 0;
                ...
                if (needFilterIndexSorted) {
                    [sectionItems sortWithOptions:NSSortConcurrent usingComparator:^NSComparisonResult(NSIndexPath* obj1, NSIndexPath* obj2) {
    ...
    if (needSplit) {
        ...
        id (*splitFun)(__strong id,SEL,id,id) = (id (*)(__strong id, SEL, id, id))(class_getMethodImplementation(self.class, sel));
    ...
    if (needSorted && !needFilterIndexSorted) {
        ...
        NSComparisonResult (*sortFun)(__strong id,SEL,id, id) = (NSComparisonResult (*)(__strong id,SEL,id, id))(class_getMethodImplementation(self.class, sortSel));
        for (NSMutableArray* items in newList) {
            [items sortWithOptions:NSSortStable usingComparator:^NSComparisonResult(id obj1, id obj2) {
...
-(NSUInteger)filter:(id)value key:(NSString*)key {
    NSUInteger (^filterImp)(NSString*, id) = ^NSUInteger(NSString* key, id value) {
        ...
        NSStringCompareOptions cmpMask = (_enableRegExpFileter ? (NSRegularExpressionSearch|NSCaseInsensitiveSearch) : NSCaseInsensitiveSearch);
