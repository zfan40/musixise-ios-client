...
typedef NSUInteger (^filters)(id value, NSString* key);         //返回 指定key过滤后的结果（即搜索出的结果的位置，未搜索到，返回 ：NSNotFound）
...
@property(nonatomic,copy) NSString* patterns;                   //指定过滤的模式串，其与enableConverPinYinFilter 、enableRegExpFileter 有关
...
@property(nonatomic,assign)BOOL groupOrderedDescending;         //指定按降序分组，需enableGroupOrdered 为yes生效, default = false 即升序
