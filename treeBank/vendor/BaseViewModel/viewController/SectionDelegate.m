...
-(void)registerCls:(Class)cls forSection:(NSInteger)section {
    if (!_sectionClsMap) _sectionClsMap = [NSMutableDictionary dictionary];
...
-(void)registerCls:(Class)cls forModel:(Class)modelCls {
    if(!_clsMap) _clsMap = [NSMutableDictionary dictionary];
...
-(XBaseSectionDelegate*)proxy:(NSInteger)section {
    ...
    if (!result) {
        ...
        if (!clsName) clsName = [_clsMap objectForKey:modelClsName];
        ...
        if (result) {
            ...
            if (block) block(section, result);
            id proxy = [[BaseProxySectionDelegate alloc] initWithTarget:nil proxy:result];
            if (!_proxyMap) _proxyMap = [NSMutableDictionary dictionary];
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    [[self proxy:indexPath.section] tableView:tableView willDisplayCell:cell forRowAtIndexPath:[self maptoSource:indexPath]];
}
...
- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath*)indexPath  {
    [[self proxy:indexPath.section] tableView:tableView didEndDisplayingCell:cell forRowAtIndexPath:[self maptoSource:indexPath]];
}
...
//    return [[self proxy:indexPath.section] tableView:tableView estimatedHeightForRowAtIndexPath:[self maptoSource:indexPath]];
//}
...
//    return [[self proxy:indexPath.section] tableView:tableView accessoryTypeForRowWithIndexPath:[self maptoSource:indexPath]];
//}
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView accessoryButtonTappedForRowWithIndexPath:[self maptoSource:indexPath]];
}
...
- (BOOL)tableView:(UITableView *)tableView shouldHighlightRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView shouldHighlightRowAtIndexPath:[self maptoSource:indexPath]];
}
...
- (void)tableView:(UITableView *)tableView didUnhighlightRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [[self proxy:indexPath.section] tableView:tableView didUnhighlightRowAtIndexPath:[self maptoSource:indexPath]];
}
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView editingStyleForRowAtIndexPath:[self maptoSource:indexPath]];
}
- (NSString *)tableView:(UITableView *)tableView titleForDeleteConfirmationButtonForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView titleForDeleteConfirmationButtonForRowAtIndexPath:[self maptoSource:indexPath]];
}

// Controls whether the background is indented while editing.  If not implemented, the default is YES.  This is unrelated to the indentation level below.  This method only applies to grouped style table views.
- (BOOL)tableView:(UITableView *)tableView shouldIndentWhileEditingRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView shouldIndentWhileEditingRowAtIndexPath:[self maptoSource:indexPath]];
}

// The willBegin/didEnd methods are called whenever the 'editing' property is automatically changed by the table (allowing insert/delete/move). This is done by a swipe activating a single row
- (void)tableView:(UITableView*)tableView willBeginEditingRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView willBeginEditingRowAtIndexPath:[self maptoSource:indexPath]];
}
- (void)tableView:(UITableView*)tableView didEndEditingRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView didEndEditingRowAtIndexPath:[self maptoSource:indexPath]];
}
...
- (NSIndexPath *)tableView:(UITableView *)tableView targetIndexPathForMoveFromRowAtIndexPath:(NSIndexPath *)sourceIndexPath toProposedIndexPath:(NSIndexPath *)proposedDestinationIndexPath {
    return [[self proxy:sourceIndexPath.section] tableView:tableView targetIndexPathForMoveFromRowAtIndexPath:[self maptoSource:sourceIndexPath] toProposedIndexPath:[self maptoSource:proposedDestinationIndexPath]];
}

// Indentation

- (NSInteger)tableView:(UITableView *)tableView indentationLevelForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView indentationLevelForRowAtIndexPath:[self maptoSource:indexPath]];
}
...
- (BOOL)tableView:(UITableView *)tableView canPerformAction:(SEL)action forRowAtIndexPath:(NSIndexPath *)indexPath withSender:(id)sender {
    return [[self proxy:indexPath.section] tableView:tableView canPerformAction:action forRowAtIndexPath:[self maptoSource:indexPath] withSender:sender];
}
- (void)tableView:(UITableView *)tableView performAction:(SEL)action forRowAtIndexPath:(NSIndexPath *)indexPath withSender:(id)sender {
    return [[self proxy:indexPath.section] tableView:tableView performAction:action forRowAtIndexPath:[self maptoSource:indexPath] withSender:sender];
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    return [[self proxy:indexPath.section] tableView:tableView commitEditingStyle:editingStyle forRowAtIndexPath:[self maptoSource:indexPath]];
}

// Data manipulation - reorder / moving support

- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath {
    return [[self proxy:sourceIndexPath.section] tableView:tableView moveRowAtIndexPath:[self maptoSource:sourceIndexPath] toIndexPath:[self maptoSource:destinationIndexPath]];
}
...
-(void)viewModelDataChanged {
    if (self.enabledUpdate) [self update];
}
-(void)update {
