...
-(UITableViewCell*)cellForClass:(Class)cls style:(UITableViewCellStyle)style kIdentifier:(NSString*)identifier {
    if(!identifier) identifier = NSStringFromClass(cls);
    UITableViewCell* cell = [self dequeueReusableCellWithIdentifier:identifier];
    if (!cell) cell = [[cls alloc] initWithStyle:style reuseIdentifier:identifier];
    
    return cell;
}
-(UITableViewCell*)cellForNibNamed:(NSString*)name kIdentifier:(NSString*)identifier {
    if(!identifier) identifier = @"xiamicell";
    UITableViewCell* cell = [self dequeueReusableCellWithIdentifier:identifier];
    if(!cell) cell = [[[NSBundle mainBundle] loadNibNamed:name owner:self options:nil] objectAtIndex:0];
    return cell;
}
@end
