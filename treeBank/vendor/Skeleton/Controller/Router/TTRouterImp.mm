...
- (instancetype)newSubRouter:(NSString *)pattern {
    ...
    [self addRoute:[NSString stringWithFormat:@"%@(.*)", pattern]
             block:^id(TTRouterContext *ctx) {
                 ctx.handled = NO;
                 BOOL a = ctx.handled;
...
- (TTRouterResult *)route:(NSString *)urlString parameters:(NSDictionary *)parameters {
    ...
    if (urlString) {
        ...
        urlString =
            [urlString stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];  //支持网络过来的URL 字符串
        urlString = [urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        urlString = fragment ? [urlString stringByAppendingString:fragment] : urlString;

        // add by pengyutang [urlString trim]，
        // 为了解决xiami://open?url=http://api.xiami.com/api/third/lotto/type/10%23login%20
        //无法创建[NSURL URLWithString]的问题
        return [self
              routeURL:[NSURL URLWithString:[urlString
...
- (TTRouterResult *)_routURL:(NSURL *)url parameters:(NSDictionary *)parameters {
    ...
    NSMutableString *urlpath = [NSMutableString stringWithCapacity:url.absoluteString.length];
    {
        ...
        if (scheme)
            [urlpath appendFormat:@"%@://", scheme];
        if ([scheme isEqualToString:@"http"] || [scheme isEqualToString:@"https"]) {
            if (!path || !path.length)
                path = @"/";
            if (host)
                [urlpath appendString:host];
            if (path)
...
- (TTRouterResult *)_routURL:(NSURL *)url
                     urlPath:(NSString *)urlpath
                   outparams:(NSDictionary *)outparams
                     handler:(BOOL *)bHandled {
    NSError *err = nil;
    ...
    for (NSString *key in [patternDic allKeys]) {
        ...
        if (re && !err) {
            NSTextCheckingResult *firstMatch =
                [re firstMatchInString:urlpath options:0 range:NSMakeRange(0, urlpath.length)];
            if (firstMatch) {
                ...
                @try {
                    TTRouterContext *context =
                        [TTRouterContext contextWithUrl:url matchPaths:array queryParam:outparams];
